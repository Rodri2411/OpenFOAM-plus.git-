...
{
    vector dir(flowDir_.value(time_.value()));
...
{
    vector dir(zDir_.value(time_.value()));
    const scalar magDir = mag(dir);
...
{
    const scalar Uref = Uref_.value(time_.value());
    const scalar Zref = Zref_.value(time_.value());
...
{
    const scalarField zGround(zGround_->value(time_.value()));
    const scalarField z0(max(z0_->value(time_.value()), ROOTVSMALL));

    scalarField Un
    (
        (Ustar(z0)/kappa_)
       *log(((zDir() & p) - zGround + z0)/z0)
    );
