...
)
{
    ...
    count += writeVolFields<scalar>(proxy, allFieldNames);
    count += writeVolFields<vector>(proxy, allFieldNames);
    count += writeVolFields<sphericalTensor>(proxy, allFieldNames);
    count += writeVolFields<symmTensor>(proxy, allFieldNames);
    count += writeVolFields<tensor>(proxy, allFieldNames);
...
bool Foam::functionObjects::ensightWrite::write()
{
    ...
    IOobjectNames objNames(mesh_, selectFields_);

    Log << type() << " " << name() << " write: (";
    writeAllVolFields(meshSubset_, objNames);
