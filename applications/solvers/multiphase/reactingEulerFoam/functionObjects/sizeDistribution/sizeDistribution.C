...
>
Foam::functionObjects::sizeDistribution::regionTypeNames
{
...
>
Foam::functionObjects::sizeDistribution::functionTypeNames
{
...
>
Foam::functionObjects::sizeDistribution::abszissaTypeNames
{
...
)
{
        ...
        default:
        {
            FatalErrorInFunction
               << "Unknown function type. Valid function types are:"
                << functionTypeNames << nl << exit(FatalError);
        }
    }
        ...
        default:
        {
            FatalErrorInFunction
               << "Unknown abszissa type. Valid abszissa types are:"
                << abszissaTypeNames << nl << exit(FatalError);
        }
    }
    ...
    if (nCells_ == 0)
    {
        FatalErrorInFunction
            << type() << " " << name() << ": "
            << regionTypeNames[regionType_] << "(" << regionName_ << "):" << nl
        ...
        default:
        {
            FatalErrorInFunction
               << "Unknown region type. Valid region types are:"
                << regionTypeNames << nl << exit(FatalError);
        }
    }
...
:
    ...
    logFiles(obr_, name),
    dict_(dict),
    regionType_(regionTypeNames.read(dict.lookup("regionType"))),
    regionName_(word::null),
    functionType_(functionTypeNames.read(dict.lookup("functionType"))),
    abszissaType_(abszissaTypeNames.read(dict.lookup("abszissaType"))),
    nCells_(0),
    cellId_(),
    volume_(0.0),
