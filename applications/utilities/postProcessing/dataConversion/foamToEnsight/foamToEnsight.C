...
int main(int argc, char *argv[])
{
    ...
    (
        IOobjectList(mesh, timeDirs.last().name()),
        fieldPatterns,
        !fieldPatterns.empty()
    );
    ...
    {
        wordHashSet supported(vtk::fieldTypes::vols);
        supported.insert(vtk::fieldTypes::dims);

        objNames.retainClasses(supported);
    }
    ...
    checkData(meshProxy.baseMesh(), timeDirs, objNames);
